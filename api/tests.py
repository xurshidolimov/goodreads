...
class BookReviewAPITestCase(APITestCase):
    def setUp(self):
        self.user = CustomUser.objects.create(username="xurshid", first_name="xurshid")
        self.user.set_password("turkiston")
        self.user.save()
    ...
    def test_book_review_detail(self):
        ...
        br=BookReview.objects.create(book=book, user=self.user, stars_given=5, comment="very nice")
        ...
        self.assertEqual(response.data['comment'], 'very nice')
        ...
        self.assertEqual(response.data['user']['username'], 'xurshid')
        self.assertEqual(response.data['user']['first_name'], 'xurshid')
        self.assertEqual(response.data['user']['id'], self.user.id)


    def test_delate_review(self):
        book=Book.objects.create(title='Book1', description='Description1', isbn='111111')
        br=BookReview.objects.create(book=book, user=self.user, stars_given=5, comment="very nice")
    ...
    def test_put_review(self):
        ...
        response=self.client.put(
            ...
            data={'stars_given':4, 'comment':"ok book", 'user_id':self.user.id, 'book_id':book.id}
        )
        br.refresh_from_db()

        self.assertEqual(response.status_code, 200)
        self.assertEqual(br.stars_given, 4)
        self.assertEqual(br.comment, 'ok book')
    ...
    def test_book_review_list(self):
        user_two = CustomUser.objects.create(username="xurshidd", first_name="xurshid")
        book=Book.objects.create(title='Book1', description='Description1', isbn='111111')
        br=BookReview.objects.create(book=book, user=self.user, stars_given=5, comment="very nice")
        br_two=BookReview.objects.create(book=book, user=user_two, stars_given=3, comment="not nice")
